...
\small{Table \ref{tab:main}(a): Results on QAMR-SUBSET dataset.}\\\vspace{2mm}
	\resizebox{1.85\columnwidth}{!}{%resize the table
		\begin{tabular}{|l|c|c|c|c|cccccccc|}
			\hline
       \textbf{Method} & \textbf{Loss} & \textbf{Discourse} & \textbf{Target Word} 
...
\small{Table \ref{tab:main}(b): Results on PTB-SUBSET dataset.}\\\vspace{2mm}
...
\small{Table \ref{tab:main}(c): Transfer test results on PTB-SUBSET dataset.}\\\vspace{2mm}
	\end{center}
...
\kechen{5\% QA pairs are filtered out from the original dataset because they contain zero  or excessive number of discourse markers, and the two corpora in our experiments are referred as \textsc{QAMR-SUBSET} and \textsc{PTB-SUBSET}. The average number of segments and words per segment are 4.7/5.7 and 4.4/6.0 for each dataset.}
